...
describe('TransformManySimpleFixture', function() {
    ...
    beforeEach(() => {
        ...
        _results = asAggregator(_source.connect()
            .pipe(
                transformMany(p => p.relations, p => p.name),
            ),
        );
    });
    ...
    it('Adds', () => {
        const parent = new PersonWithChildren('parent', 50, [
            new Person('Child1', 1),
            new Person('Child2', 2),
            new Person('Child3', 3),
        ]);
    ...
    it('removewithIncompleteChildren', () => {
        const parent1 = new PersonWithChildren('parent', 50, [

            new Person('Child1', 1), new Person('Child2', 2), new Person('Child3', 3),
        ]);
        _source.addOrUpdate(parent1);

        const parent2 = new PersonWithChildren('parent', 50, [
