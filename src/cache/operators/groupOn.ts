...
export function groupOn<TObject, TKey, TGroupKey>(
    ...
    return function groupOnOperator(source) {
        ...
        function regroup() {
            ...
            const items = ixFrom(_itemCache.entries()).pipe(ixMap(([key, value]) => Change.refresh(key, value.item)));
        ...
        function handleUpdates(changes: ArrayOrIterable<Change<TObject, TKey>>, isRegrouping = false) {
            ...
            grouped.forEach(group => {
                ...
                if (created) {
                    result.push(Change.add(group.key, groupCache));
                }
                                ...
                                if (previous.groupKey !== current.groupKey) {
                                    ...
                                    if (g !== undefined) {
                                        ...
                                        if (g.size === 0) {
                                            ...
                                            result.push(Change.remove(g.key, g));
                                        }
                                    }
                ...
                if (groupCache.cache.size === 0) {
                    ...
                    result.push(Change.remove(group.key, groupCache));
                }
            });
