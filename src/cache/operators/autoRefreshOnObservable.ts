import { Npc } from '../../notify/notifyPropertyChanged';
...
export function autoRefreshOnObservable<TObject, TKey>(
    reevaluator: (value: Npc<TObject>, key: TKey) => Observable<unknown>,
    ...
    return function autoRefreshOnObservableOperator(source) {
        return new Observable<IChangeSet<Npc<TObject>, TKey>>(observer => {
            const shared: ConnectableObservable<IChangeSet<Npc<TObject>, TKey>> = source.pipe(publish()) as any;

            //monitor each item observable and create change
            let changes = shared
                .pipe(mergeMany((t, k) =>
                    reevaluator(t, k)
                        .pipe(map(_ => new Change<Npc<TObject>, TKey>('refresh', k, t))),
                ));

            //create a changeset, either buffered or one item at the time
            let refreshChanges: Observable<IChangeSet<Npc<TObject>, TKey>>;
            if (changeSetBuffer === undefined) {
                refreshChanges = changes.pipe(map(c => new ChangeSet<Npc<TObject>, TKey>([c])));
            } else {
                refreshChanges = changes
                    .pipe(
                        ...
                        map(items => new ChangeSet<Npc<TObject>, TKey>(items)),
                    );
            }
